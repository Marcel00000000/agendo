    ...
    return {
      visitor: {
        JSXElement(path: NodePath<JSXElement>) {
          ...
          const bodyPaths: NodePath<t.Statement>[] = Array.isArray(body)
            ? (body as NodePath<t.Statement>[])
            : [];
          ...
          if (!alreadyImported) {
            ...
            if (firstImport === -1) {
              (program as unknown as {
                unshiftContainer: (key: string, node: t.Statement) => void;
              }).unshiftContainer('body', importDecl as t.Statement);
            } else {
              bodyPaths[firstImport].insertBefore(importDecl);
            }
          }
